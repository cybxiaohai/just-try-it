  ...
  <div>
    ...
    <div class="page-panel log-panel">
      ...
      <el-table :data="caseStore.permissionLogs" border stripe>
        <el-table-column prop="time" label="何时" min-width="150" />
        <el-table-column prop="user" label="谁" min-width="120" />
        <el-table-column prop="action" label="做了什么" min-width="240" />
      </el-table>
    </div>
  </div>
</template>
...
const permissionRows = ref([
  ...
  { role: '案管', menus: [...menuOptions], locked: false },
  { role: '领导', menus: ['数据大盘'], locked: true }
])
